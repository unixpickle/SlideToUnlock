...
- (void)drawText:(NSString *)text atPoint:(BMPoint)point font:(UIFont *)theFont color:(BMPixel)pixelColor {
	...
	CGContextTranslateCTM([self context], 0, 0 - [self bitmapSize].y);
	CGContextSetRGBFillColor([self context], pixelColor.red, pixelColor.green, pixelColor.blue, pixelColor.alpha);
	[text drawAtPoint:CGPointMake(point.x, point.y) withFont:theFont];
	// }
